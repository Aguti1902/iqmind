  ...
  try {
    ...
    if (!userEmail || !adminEmails.includes(userEmail)) {
      return NextResponse.json(
        { error: 'No autorizado. Solo admin puede crear usuarios de prueba.' },
        { status: 401 }
      )
    }
    ...
    if (existing) {
      ...
      return NextResponse.json({
        ...
        credentials: {
          ...
          url: 'https://iqmind.mobi/es/login'
        },
        note: 'Usuario actualizado con nueva contraseña'
      })
    }
    ...
    return NextResponse.json({
      ...
      note: 'Este es un usuario normal (NO admin) sin suscripción premium'
    })
